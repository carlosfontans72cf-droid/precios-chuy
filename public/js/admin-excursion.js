...
import { requireRole, getSession } from './session.js';
import {
...
if (!session) {
  // requireRole ya redirigió al login
  console.log('Redirigido al login por falta de permisos');
} else {
  const userId = session.userId;
  const userEmail = session.userEmail;
  console.log('✅ Admin Excursión cargado:', { userId, userEmail });
  ...
  async function verificarAprobacion() {
    try {
      ...
      if (!userDoc.empty) {
        ...
        console.log('Datos del usuario:', { aprobado: data.aprobado, role: data.role });
        
        if (!data.aprobado) {
          const aprovDiv = document.getElementById('aprobacion-pendiente');
          const panelDiv = document.getElementById('panel-activo');
          if (aprovDiv) aprovDiv.style.display = 'block';
          if (panelDiv) panelDiv.style.display = 'none';
          console.log('Usuario no aprobado');
        } else {
          const aprovDiv = document.getElementById('aprobacion-pendiente');
          const panelDiv = document.getElementById('panel-activo');
          if (aprovDiv) aprovDiv.style.display = 'none';
          if (panelDiv) panelDiv.style.display = 'block';
          console.log('Usuario aprobado, cargando panel...');
          loadExcursiones();
          loadReservas();
          loadStats();
        }
